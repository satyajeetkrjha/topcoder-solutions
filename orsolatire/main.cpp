...
int res=n;
for(int i=0;i<31;i++){
    if(goal & (1<<i))// if ith bit is set in goal
    {
        int c=0;// count all those numbers in which this bit is also set
        for(int j=0;j<n;j++){
            int x=numbers[j];// got one element of the vector ,now just check it's ith bit
            if((x &(1<<i)) && ((goal & x)==x)){
                c++;
            }
